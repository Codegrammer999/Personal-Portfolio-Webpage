...
<template>
    <div>
        ...
        <div class="px-6 py-8 lg:max-w-4xl mx-auto">
            <h1 class="text-4xl font-bold text-center mb-4 gradient-text">Let's Connect</h1>
            <p class="text-xl text-center text-slate-300 mb-12">Ready to bring your ideas to life? Let's start a conversation.</p>
            
            <div ref="emailMsgEl" class="text-center">
                <p class="modern-card p-4 bg-gradient-to-r from-green-500 to-teal-500 text-white rounded-2xl text-lg font-medium" v-if="emailMsg"> {{ emailMsg }}</p>
            </div>
            
            <div class="modern-card p-8 rounded-2xl mb-8">
                <form @submit.prevent="sendMail" ref="form" class="space-y-6">
                    <div class="flex flex-col">
                        <label for="name" class="text-lg font-semibold mb-3 text-white cursor-pointer">Name</label>
                        <input 
                            type="text" 
                            id="name" 
                    ...
                    <div class="flex flex-col">
                        <label for="email" class="text-lg font-semibold mb-3 text-white cursor-pointer">Email</label>
                        <input 
                            type="email" 
                            id="email" 
                            class="p-4 bg-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-300 text-white placeholder-slate-400" 
                    ...
                    <div class="flex flex-col">
                        <label for="message" class="text-lg font-semibold mb-3 text-white cursor-pointer">Message</label>
                        <textarea 
                            id="message" 
                    ...
                    <p v-if="formData.error" class="text-red-400 font-medium">{{ formData.error }}</p>
                    
                    <div class="text-center">
                        <div class="modern-btn w-full flex justify-center items-center space-x-3 py-4" v-if="isProcessing">
                            <div class="animate-spin w-6 h-6 border-2 border-white rounded-full border-t-transparent"></div>
            ...
            <div class="modern-card p-8 rounded-2xl text-center">
                <h2 class="text-2xl font-bold mb-4 gradient-text">Alternative Ways to Reach Me</h2>
                <p class="text-slate-300 mb-8 leading-relaxed">
                    Prefer a different communication method? No problem! Here are other ways to get in touch.
                </p>
